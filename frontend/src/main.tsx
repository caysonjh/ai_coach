...
function CoachResponseDetails({
  ...
  return (
    <div className="coachResult">
      ...
      <small>{response.used_ollama ? "Generated by local Ollama." : "Deterministic local preview. ChatGPT actions should handle the live coach experience."}</small>
    </div>
  );
}
...
function ChatGPTPanel() {
  ...
  return (
    <Panel title="ChatGPT Actions" icon={<Brain />}>
      <div className={`ollamaStatus ${status?.enabled ? "online" : "offline"}`}>
        <strong>{status?.enabled ? "Configured" : "Not Configured"}</strong>
        <span>{status?.auth_required ? "Bearer token required for action calls." : "Auth not required in local dev."}</span>
      </div>
      <div className="connector">
        <strong>Public base URL</strong>
        <span>{status?.public_base_url || "Set CHATGPT_PUBLIC_BASE_URL for a deployed backend."}</span>
        <small>Use this URL in your Custom GPT action configuration.</small>
      </div>
      <div className="connector">
